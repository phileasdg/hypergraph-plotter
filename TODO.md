...
## 4. Interactive node pinning/locking [Implemented]
- [x] Enable pinning/locking nodes in place to control layouts manually while the physics engine is running.
- [x] Allow users to drag a node to a custom location and lock it in place upon dropping it (e.g. locking nodes a, b, c), while other unlocked nodes continue to dynamically adjust their positions based on spring embedding forces.
- [x] Add visual indicators for locked nodes (e.g. a small pin badge or a dashed outline style).
