...
If a node's label exceeds 80 characters, it is truncated to 77 characters followed by an ellipsis (`...`) for layout, rendering, and bounding-box sizing:
$$L_{\text{display}} = \begin{cases}
L & \text{if } |L| \le 80 \\
L[0..76] + \text{"..."} & \text{if } |L| > 80
\end{cases}$$
...
This reduces to:
$$N^* = \sqrt{|L| \times 2.18}$$
The value is clamped to yield the final character wrap threshold:
$$\text{charsPerLine} = \max(8, \min(22, \lfloor N^* \rceil))$$

### B. Shape Selection and Sizing
