...
class BarrierGoalLayout extends BipartiteForceLayout {
  constructor(options = {}) {
    super(options);
  }
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let j = i + 1; j < n; j++) {
        ...
        if (n1.componentId === n2.componentId) {
          ...
          // Inter-component repulsion
          // Skip hubs to prevent curve distortion and physics feedback loops
          if (n1.isHub || n2.isHub) continue;

          const goal = this.componentSpacing;
          if (goal <= 0) continue;
          if (d < goal) {
            // Barrier force: smoothly goes to 0 at d = goal
            force = this.kRepel * (1 / d - 1 / goal);
          }
        }
...
function runTest() {
  const layout = new BarrierGoalLayout({
    width: 800,
    height: 600,
    kAttract: 0.07,
    kRepel: 2400,
  ...
  console.log("Testing Barrier Repulsion Model Convergence:");
