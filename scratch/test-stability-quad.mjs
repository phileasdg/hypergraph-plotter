...
// Custom layout implementing a quadratic separation force:
// force = k * (goal - d)^2 / goal
class QuadraticGoalLayout extends BipartiteForceLayout {
  constructor(options = {}) {
    super(options);
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let j = i + 1; j < n; j++) {
        ...
        if (n1.componentId === n2.componentId) {
          ...
          if (goal <= 0) continue;
          if (d < goal) {
            // Quadratic separation force:
            const diff = goal - d;
            force = this.kComponentOverlap * (diff * diff) / goal;
          }
        }
...
function testStability(coeff) {
  const layout = new QuadraticGoalLayout({
    width: 800,
    height: 600,
    kAttract: 0.07,
    kRepel: 2400,
  ...
  console.log(`Quad Coeff: ${coeff.toString().padEnd(4)} | Avg Node Speed at Tick 500: ${avgSpeed.toFixed(5)}px/frame | Min Inter-Comp Dist: ${Math.round(minInterDist)}px`);
}

console.log("Evaluating quadratic overlap force model stability...");
testStability(2.0);
testStability(1.5);
testStability(1.0);
testStability(0.5);
