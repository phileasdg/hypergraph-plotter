...
// Custom layout class allowing us to configure the inter-component repulsion coefficient
class TunedGoalLayout extends BipartiteForceLayout {
  constructor(options = {}) {
    super(options);
    this.kComponentOverlap = options.kComponentOverlap !== undefined ? options.kComponentOverlap : 0.8;
  }
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let j = i + 1; j < n; j++) {
        ...
        if (n1.componentId === n2.componentId) {
          ...
          if (goal <= 0) continue;
          if (d < goal) {
            // Apply the inter-component separation force with our tuned coefficient
...
function testStability(coeff) {
  const layout = new TunedGoalLayout({
    width: 800,
    height: 600,
    kAttract: 0.07,
    kRepel: 2400,
  ...
  // Run simulation and track speeds over time
  for (let tick = 1; tick <= 500; tick++) {
    layout.tick();
  }

  // Calculate average node speed at tick 500
  let totalSpeed = 0;
  ...
  // Calculate minimum inter-component node distance to verify spacing is maintained
  const compNodes = {};
  ...
  console.log(`Coefficient: ${coeff.toString().padEnd(4)} | Avg Node Speed at Tick 500: ${avgSpeed.toFixed(5)}px/frame | Min Inter-Comp Dist: ${Math.round(minInterDist)}px`);
}

console.log("Evaluating stability / jittering across different overlap force coefficients...");
testStability(0.8);
testStability(0.5);
testStability(0.3);
testStability(0.15);
